...
class WC_Jilt_Cart_Handler {
	...
	private function set_address_fields_from_customer( $params ) {
		...
		foreach ( WC_Jilt_Order::get_jilt_order_address_mapping() as $wc_field => $jilt_field ) {
			...
			if ( Framework\SV_WC_Plugin_Compatibility::is_wc_version_gte_3_0() ) {
				...
				$shipping_value = is_callable( array(
					WC()->customer,
					$shipping_method
				) ) ? WC()->customer->$shipping_method() : null;
	...
	private function has_customer_data_from_js_api() {
		...
		if ( Framework\SV_WC_Plugin_Compatibility::is_wc_version_gte_3_0() ) {
			...
			return ( WC()->customer->first_name || WC()->customer->last_name || WC()->customer->email );
		}
	}
	...
	private function get_tax_lines( $item ) {
		...
		return array(
			array(
				'amount' => $this->amount_to_int( $item['line_tax'] ),
			),
		);
	}
