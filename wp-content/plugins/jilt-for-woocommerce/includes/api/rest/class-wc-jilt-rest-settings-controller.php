...
class WC_Jilt_REST_Settings_Controller extends WC_REST_Controller {
	...
	public function get_items( $request ) {
		...
		$safe_settings     = $this->get_safe_settings( wc_jilt()->get_integration()->get_settings() );
	...
	public function update_items( $request ) {
		...
		foreach ( $request->get_params() as $setting => $value ) {
	...
	private function prepare_items_for_response( $settings ) {
		...
		return rest_ensure_response( $this->get_safe_settings( $settings ) );
	}


	/**
	 * Gets the settings that are safe for updating and viewing.
	 *
	 * @since 1.5.0
	 *
	 * @param array $settings settings items
