...
class WC_Jilt_Integration_Admin {
	...
	public function process_admin_options() {
		...
		if ( isset( $_POST['woocommerce_jilt_disconnect'] ) || ( isset( $_POST['woocommerce_jilt_secret_key'] ) && empty( $_POST['woocommerce_jilt_secret_key'] ) ) ) {
			...
			$integration->clear_connection_data(); // this will wipe Jilt oauth tokens and client credentials (and the secret key) in WC

			// remove the Jilt for WC webhooks & REST API key
			...
			if ( $integration->get_secret_key() ) {
				...
				unset( $_POST['woocommerce_jilt_secret_key'], $integration->settings['secret_key'] );

				$integration->set_secret_key( '' );
