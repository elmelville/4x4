...
class WC_Jilt extends Framework\SV_WC_Plugin {
	...
	public function includes() {
		...
		require_once( $this->get_plugin_path() . '/includes/integrations/abstract-wc-jilt-integration-base.php' );
		require_once( $this->get_plugin_path() . '/includes/integrations/class-wc-jilt-product-bundles-integration.php' );
		require_once( $this->get_plugin_path() . '/includes/integrations/class-wc-jilt-composite-products-integration.php' );
		require_once( $this->get_plugin_path() . '/includes/integrations/class-wc-jilt-gift-cards-integration.php' );
		require_once( $this->get_plugin_path() . '/includes/integrations/class-wc-jilt-paypal-standard-integration.php' );
		require_once( $this->get_plugin_path() . '/includes/integrations/class-wc-jilt-advanced-access-manager-integration.php' );
		$this->integrations = $this->load_class( '/includes/integrations/class-wc-jilt-integrations.php', 'WC_Jilt_Integrations' );
	}
	...
	public function add_delayed_admin_notices() {
		...
		try {
			...
			$this->get_admin_notice_handler()->add_admin_notice(
				__( 'Jilt works best with the PHP function random_bytes(), which is not available on your site. Please ask your hosting provider to assist with this', 'jilt-for-woocommerce' ),
	...
	public function get_installation_id() {
		...
		$installation_id = get_option( 'wc_jilt_installation_id' );
	...
	public function generate_installation_id() {
		...
		$installation_id = strtolower( $this->generate_random_token( 64 ) );

		update_option( 'wc_jilt_installation_id', $installation_id );
