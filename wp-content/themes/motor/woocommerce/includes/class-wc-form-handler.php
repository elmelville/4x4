				...
				foreach ( $address as $key => $field ) {					
					...
					if( $update_all == true ){
						if(strpos( $key, "billing" ) !== false){
							$shipping_key = str_replace("billing","shipping",$key);
						}else{
							$shipping_key = str_replace("shipping","billing",$key);
						}
						
						if ( is_callable( array( $customer, "set_$shipping_key" ) ) ) {
							$customer->{"set_$shipping_key"}( wc_clean( $_POST[ $key ] ) );
						} else {
							$customer->update_meta_data( $shipping_key, wc_clean( $_POST[ $key ] ) );
						}

						if ( WC()->customer && is_callable( array( WC()->customer, "set_$shipping_key" ) ) ) {
							WC()->customer->{"set_$shipping_key"}( wc_clean( $_POST[ $key ] ) );
						}					
					}
				}
				$customer->save();
